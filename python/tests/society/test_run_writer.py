...
from town_core.society.run import AUTHORITY_KIND_FILENAMES, SocietyRunWriter
...
def test_writer_serializes_each_envelope_once_and_reuses_exact_line_bytes(
    tmp_path: Path,
    catalog: CatalogBundle,
    m3_catalogs: M3Catalogs,
    ...
    monkeypatch.setattr("town_core.society.run.canonical_json", counted_canonical_json)
    ...
    for sequence, raw_record in enumerate(result.authority_records, start=1):
        ...
        line = real_canonical_json(envelope).encode("utf-8") + b"\n"
        expected_authority.extend(line)
        expected_by_kind[kind].extend(line)

    assert canonical_calls == len(result.authority_records)
