...
的 7 日与 30 日规则 Soak 中稳定运行和权威回放。M4 已启动，规则教师原始数据
已完成并通过质量审查，当前正在实现 300 条哈希链社会锚点的选择与独立复核；
长训练尚未开始，M5 DeepSeek 仍未启动。
...
| M4 社会锚点与世界模型 | 进行中 | 已审计 4090 云环境；实现可暂停、可回退的小型社会 Outcome Model |
...
- M4 已于 2026-08-04 从接受的 M3 基线启动；
- 已加入版本化 feature/label/manifest 契约、只读决策捕获、规则教师批量适配器和外置 Parquet 生成器；
- 五 seed 原始矩阵已形成 499,978 行、71,636 个完整决策组，并通过独立质量门；
- ADR-0013 冻结 300 条社会锚点的任务、Codex 判断、独立审查与 holdout 哈希链；
- 数据生成与存储说明见 [`docs/model/M4_DATASET.md`](docs/model/M4_DATASET.md)；
- 生成程序化规则样本；
...
- AutoDL NVIDIA RTX 4090 24GB、16 CPU、120GB 内存；
- Ubuntu 22.04、Python 3.12.3、PyTorch 2.5.1+cu124、CUDA 12.4；
- M4 数据 extra 固定 `pyarrow==19.0.1`；
- 30GB 系统盘、50GB 云端本地工作空间、200GB 已挂载文件存储；
- 用于训练集、检查点、评估和导出模型；
- GPU 实算、SSH 密钥、重启持久性和 GitHub 网络加速均已在 M4 入口验证；
- 生成数据、权重和运行日志不进入 Git，耐久产物保存在文件存储并以哈希引用。
